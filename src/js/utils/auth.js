...
export const auth = {
  ...
  async logout() {
    try {
      ...
      // Игнорируем ошибку — всё равно удаляем токен локально
    }
...
/**
 * Периодическая проверка и продление сессии на сервере
 */
export function setupSessionAutoExtend() {
  const interval = SESSION_CONFIG.checkInterval || 30 * 60 * 1000;

  setInterval(async () => {
    const valid = await auth.checkSession();
    if (!valid && auth.isAuthenticated()) {
      // Токен был, но сервер его не принял — разлогиниваем
      await auth.logout();
      window.location.href = 'login.html';
    }
  }, interval);
